...
## Transkriptomische Entzündungs-, Effektoren- und Ribosomen-Scores nach TAVI

Dieser Codeblock berechnet **RNA-basierte Scores**, die zentrale immunologische und zelluläre Prozesse nach Transkatheter-Aortenklappenimplantation (TAVI) quantifizieren. Ziel ist es, die **Stärke und Dynamik der systemischen Entzündungsantwort** sowie der nachfolgenden **immunologischen Reprogrammierung** auf Patientenebene abzubilden.

Basierend auf variance-stabilisierter Expression (VST) werden drei biologisch motivierte Signaturen definiert:\
(1) ein **akuter inflammatorischer Score**, der die frühe angeborene Immunantwort (z. B. neutrophile Aktivierung und Zytokinproduktion) widerspiegelt,\
(2) ein **immunologischer Effektorscore**, der zytotoxische T-/NK-Zell-Programme erfasst, sowie\
(3) ein **ribosomaler Aktivitätsscore**, der globale Veränderungen der Proteinbiosynthese als Marker zellulären Stresses und funktioneller Reprogrammierung abbildet.

Die Scores werden für jedes Sample berechnet, entlang der Zeitpunkte (A → V1 → V2) visualisiert und erlauben eine **quantitative Beschreibung interindividueller Unterschiede** in der inflammatorischen Antwort auf TAVI. Sie bilden damit die Grundlage für weiterführende Analysen, einschließlich Korrelationen mit klinischen Parametern und der Identifikation potenzieller Biomarker.
...
# -----------------------------
# 1) Row-scale (z-score pro Gen über Samples)
# -----------------------------
...
# ============================================================
# Gene-Sets für Scores aus ORA-RDS rekonstruieren
...
# ---- Settings müssen zu deinem ORA-Run passen
padj_cutoff <- if (exists("padj_cutoff")) padj_cutoff else 0.05
lfc_min     <- if (exists("lfc_min")) lfc_min else 0.5
...
#     und zwei weitere aus zufälligen Dritteln – ABER das wollen wir nicht.
...
if (!have_split_lists) {
  message("Hinweis: genes_go_eff / genes_kegg_ribo / genes_kegg_sig nicht gefunden -> nutze Fallback (alle Gene für alle Scores).")
  ...
  # Deshalb: wir bauen Sets über die bereits gemappte ENS-Liste genes_present2
  # indem wir schlicht die präsentierten Gene verwenden, die aus den jeweiligen Quellen stammen:
  # -> am robustesten: wir remappen die Split-Listen wieder nach ENS, falls nötig.
  suppressPackageStartupMessages({
    library(AnnotationDbi)
    library(org.Hs.eg.db)
  })
...
# -----------------------------
# 3) Scores berechnen (mean z über Gene)
# -----------------------------
