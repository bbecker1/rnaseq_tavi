...
Dieser Chunk führt QC-Analysen durch: Varianz-stabilisierende Transformation (VST), PCA-Plots (PC1/2 und PC3/4) zur Visualisierung der Sample-Ähnlichkeit, sowie eine Korrelations-Heatmap.
...
# Varianz-stabilisierende Transformation (blind = TRUE für QC)
...
for (name in names(wald_results)) {
  ...
  DESeq2::plotMA(wald_results[[name]], ylim = c(-2, 2), main = paste(label, "– unshrunken"))
  DESeq2::plotMA(shrink_results[[name]], ylim = c(-2, 2), main = paste(label, "– shrunken"))
}
```
...
gsva_scores <- load_or_compute(gsva_cache, function() {
  ...
  if (gsva_version >= "1.40") {
    ...
    param <- gsvaParam(
      ...
      minSize = 10,
      maxSize = 500,
      kcdf = "Gaussian"  # für VST-transformierte Daten
    )
...
extract_tf_from_enrichr <- function(enrichr_result, database_name, top_n = 30,
                                     ...
                                     p_floor = 1e-50) {     # p-value Capping für numerische Stabilität
