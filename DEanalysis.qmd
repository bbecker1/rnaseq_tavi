...
**Statistical Power:** Die Stichprobengröße von n=46 biologischen Replikaten übertrifft deutlich die empirisch belegte Mindestempfehlung von n≥12 für RNA-seq Differential Expression-Studien (Schurch et al., 2016). Das longitudinale gepaarte Design (\~ pid + timepoint) erhöht die statistische Power zusätzlich durch Reduktion der Residualvarianz im Vergleich zu ungepaarten Designs (Orr & Huber, 2018). Diese Kombination aus ausreichender Stichprobengröße und paired design gewährleistet hohe statistische Sensitivität zur Detektion biologisch relevanter transkriptioneller Veränderungen.
...
vst_cache <- file.path("../../res/cache", "vst_blind_TRUE.rds")
...
Zusammenfassung der Wald-Test-Ergebnisse: Konvertierung zu Tibbles, Filterung signifikanter Gene (padj \< 0.05), Zählung up/down-regulierter Gene.
...
Weiterverarbeitung der LRT-Ergebnisse: Annotation mit SYMBOL/GENENAME, Filterung signifikanter Gene (padj \< 0.05, Top 200), und Heatmap über alle Zeitpunkte (A/V1/V2).
...
gsva_scores <- load_or_compute(gsva_cache, function() {
  ...
  if (gsva_version >= "1.40") {
    ...
    gsva(param, verbose = TRUE, BPPARAM = BiocParallel::MulticoreParam(workers = n_workers))
  } else {
    # Alte API (< 1.40)
...
#       → Mixed models liefern formale Signifikanz-Tests für Reviewer
...
if (!requireNamespace("lme4", quietly = TRUE)) {
  ...
  if (nrow(sig_pathways) > 0) {
    ...
    cat("\n⚠ GSVA: Keine signifikanten Zeitpunkt-Effekte (BH padj < 0.05)\n")
  }
}
...
p_tf_consensus <- ggplot(
  ...
  scale_color_gradient(
    ...
    low = "#fee5d9",
    high = "#a50f15",
    limits = c(0, 50)  # Capping für Visualisierung
  ) +
  labs(
    x = NULL,
...
encode_data <- top_tfs_all |>
  ...
  mutate(
    ...
    # Capping für numerische Stabilität (max 50)
...
if (nrow(ribosome_gsea) > 0) {
  ...
  cat("\n✓ GSEA confirms ribosome-related pathway enrichment\n")
  cat("  → ORA extreme p-values are BIOLOGICALLY REAL, not statistical artifacts\n")
} else {
...
cat("\n✓ Cross-method validation confirms: extreme ORA p-values are REAL.\n")
